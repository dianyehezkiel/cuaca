...
export default function CurrentWeather({ id, lat, lon }: CurrentWeatherProps) {
  ...
  React.useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        const { data: currentWeather } = await axios.get<CurrentWeatherType>('/api/weather', { params })
  ...
  if (loading) {
    ...
    >
      ...
      >
        <Skeleton height={24} width="100%" startColor="blue.100" endColor="blue.300" />
        <Skeleton height={24} width="100%" startColor="blue.100" endColor="blue.300" />
      </Box>
      <Box
        px={4}
      ...
      >
        <Box
          ...
          justifyContent='center'
        >
          <Skeleton height={8} width="100%" startColor="blue.100" endColor="blue.300" />
          <Skeleton height={8} width="100%" startColor="blue.100" endColor="blue.300" />
          <Skeleton height={8} width="100%" startColor="blue.100" endColor="blue.300" />
        </Box>
        <Box
    ...
    >
      ...
      >
        <Box textAlign='center' display='flex' flexDir='column' alignItems='center'>
          <WeatherIcon boxSize={20} weatherId={currentWeather[id].weather.id} day={day()} />
          <Text fontSize='sm' fontWeight='medium'>{currentWeather[id].weather.description}</Text>
        </Box>
        <Box textAlign='right'>
          <Text>{todayString}</Text>
          <Box>
            <Text fontSize='3xl' fontWeight='bold'>{currentWeather[id].main.temp.toFixed(1)}&deg;C</Text>
            <Box
              display='flex'
            ...
            >
              <Box display='flex' alignItems='center'>
                ...
                <Text fontSize='xs'>{currentWeather[id].main.temp_max.toFixed(1)}&deg;</Text>
              </Box>
              <Box display='flex' alignItems='center'>
                <TriangleDownIcon w={3} h={3} mr={1}/>
                <Text fontSize='xs'>{currentWeather[id].main.temp_min.toFixed(1)}&deg;</Text>
              </Box>
            </Box>
          </Box>
        </Box>
      ...
      <Box px={2} mb={{base: 'none', md: '4'}} display='flex' justifyContent='space-evenly' w='100%'>
        <Box px={2} display='flex' justifyContent='space-evenly' w='100%' alignItems='center'>
          <Box display='flex' flexDir='column' alignItems='center' flex='1'>
            <Text fontSize='sm' fontWeight='medium'>{currentWeather[id].main.feels_like.toFixed(1)}&deg;C</Text>
            <Text fontSize='xs'>Suhu Terasa</Text>
          </Box>
          <Box display='flex' flexDir='column' alignItems='center' flex='1'>
            <Text fontSize='sm' fontWeight='medium'>{currentWeather[id].main.humidity}%</Text>
            <Text fontSize='xs'>Kelembapan</Text>
          </Box>
          <Box display='flex' flexDir='column' alignItems='center' flex='1'>
            <Text fontSize='sm' fontWeight='medium'>{currentWeather[id].main.pressure} hPa</Text>
            <Text fontSize='xs'>Tekanan</Text>
          </Box>
        </Box>
        <Box px={2} display={{base: 'none', md: 'flex'}} justifyContent='space-evenly' w='100%' alignItems='center'>
          <Box display='flex' flexDir='column' alignItems='center' flex='1'>
            <Text fontSize='sm' fontWeight='medium'>{currentWeather[id].wind.speed} m/s</Text>
            <Text fontSize='xs'>Kcptn. Udara</Text>
          </Box>
          <Box display='flex' flexDir='column' alignItems='center' flex='1'>
            <Text fontSize='sm' fontWeight='medium'>{currentWeather[id].wind.deg}&deg;</Text>
            <Text fontSize='xs'>Arah Udara</Text>
          </Box>
          <Box display='flex' flexDir='column' alignItems='center' flex='1'>
            <Text fontSize='sm' fontWeight='medium'>{(currentWeather[id].visibility / 1000).toFixed(1)} km</Text>
